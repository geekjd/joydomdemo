...
export default class Agent {
  ...
  update() {
    ...
    var v = 1;
    var scale = 1;
    if (this.prefVelocity.x != null && this.prefVelocity.y != 0) {
      v = RVOMath.dot(this.prefVelocity, this._newVelocity);
    }
    if (v < 0) {
      scale = 0.01;
    }
    var a = this._newVelocity.minus(this.velocity).normalize().scale(scale * 30 * this.simulator.timeStep);
    a.x = a.x || 0;
    a.y = a.y || 0;
    this.velocity = this.velocity.plus(a);



    if (this.id == 0) {
      let d = this._newVelocity.scale(this.simulator.timeStep);
      // console.log("delta pos:" , d.x.toFixed(2) , "-" , d.y.toFixed(2), "\nvel:" , this.velocity.x.toFixed(2) , "-" , this.velocity.y.toFixed(2));
    }
    this.position = this.position.plus(this.velocity.scale(this.simulator.timeStep * scale))
  }
