...
/**多重射击 */
export default class BUFF_MultipleShot extends BuffBase {
    ...
    InitBuffBase(_SoldiersBase: any, TempDamageData: DamageData) {
        ...
        let Temp: _HeroBuffConfig.DataType = HeroBuffDataProxy.instance.GetBuffInfoByEnglishName("BUFF_MultipleShot");
    ...
    setValue() {
        if (this.BuffValueArray.length >= 1) {
            this.m_DamageData.AttackCount += this.BuffValueArray[0];
        }
    }
