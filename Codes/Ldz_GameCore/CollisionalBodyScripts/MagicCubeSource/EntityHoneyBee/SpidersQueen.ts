...
/**蜘蛛女皇 */
export default class SpidersQueen extends MagicCubeSource {


    Init(Lv: number = 1) {
        this.MagicName = "SpidersQueen";
    ...
    AttackTarget(_TargetObj: Laya.Sprite3D): boolean {
        ...
        if (this.m_AttributeBase.m_Projectiles != "") {
            ...
            this.m_DamageData.AttackPos = this.AttackPos.transform.position.clone();
            MesManager.event(ESceneEvent.ReleasePet, [this.m_AttributeBase.m_MagicMiscId, this.m_DamageData.Clone()])
        } else {
