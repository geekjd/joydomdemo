...
/**仙人掌射手 */
export default class Toon_Cactus extends MagicCubeSource {


    Init(Lv: number = 1) {
        this.MagicName = "Toon_Cactus";
        this.m_Enitiy = Enitiy.Filed_Type;
        super.Init(Lv);
        this.m_AttributeBase.m_Projectiles = "132456";
    }
    onAwake() {
    ...
    AttackTarget(_TargetObj: Laya.Sprite3D): boolean {
        if (this.m_AttributeBase.m_Projectiles != "") {
            ...
            MesManager.event(ESceneEvent.ReleaseSkills, [200002, this.m_DamageData.Clone()]);
        } else {
