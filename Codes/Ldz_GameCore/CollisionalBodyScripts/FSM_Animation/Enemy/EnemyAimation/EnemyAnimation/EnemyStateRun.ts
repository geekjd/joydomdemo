...
import { EntityState, GameGeneralAnimationDefin, WorkType } from "src/Ldz_GameCore/GeneralScripts/GameDefine";
...
export default class EnemyStateRun extends CombatantStateBase {
    ...
    public OnExecute()             //执行状态的虚函数
    {
        ...
        this.m_StateManage.LostTargetTime += (LayaUtils.deltaTime / 1000);
        if (this.m_StateManage.LostTargetTime >= 5) {
        ...
        if (this.m_StateManage.m_MagicCubeSource.m_WorkType == WorkType.HuntingType) {
            // if (this.m_StateManage.m_TargetObj != null && this.m_StateManage.m_TargetObj.parent != null && !this.m_StateManage.TargetScript.IsDie) {
        ...
        if (this.m_StateManage.m_TargetObj == null || this.m_StateManage.m_TargetObj.parent == null || this.m_StateManage.TargetScript.IsDie) {
            ...
            if (this.GetTargetDis() < this.m_StateManage.m_MagicCubeSource.m_TempAttributeBase.m_Range - 1) {
                this.m_StateManage.m_MagicCubeSource.m_FinderAgent.SetStopMove();
                this.m_StateManage.ChangeState(EntityState.Idle);
            } else {
    ...
    public OnExit()                //推出状态的虚函数
    {
        ...
        this.m_StateManage.LostTargetTime = 0;
        super.OnExit();
    }
