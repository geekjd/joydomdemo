...
export default class MeleeStateAttack3 extends CombatantStateBase {
    ...
    public Init(SelfObj: Laya.Sprite3D, _CombatantManageBase: CombatantManageBase)                  //初始化虚函数
    {
        ...
        this.m_EntityState = EntityState.AttackThree;
    }
    public OnEnter()               //进入状态的虚函数
    {
        ...
        this.PlayAnimation(GameMeleeAnimationDefin.AnimationName_MeleeRightAttack03);
    ...
    {
        //当前状态不是 这个脚本的时候 就退出
        if ((this.m_StateManage.m_CurStateBase.m_EntityState != EntityState.AttackThree)) {
            return;
        }
