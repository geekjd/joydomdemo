...
export default class MeleeStateAttack extends CombatantStateBase {
    ...
    public OnEnter()               //进入状态的虚函数
    {
        ...
        this.PlayAnimation(GameMeleeAnimationDefin.AnimationName_MeleeRightAttack01);
    ...
    public OnExecute()             //执行状态的虚函数
    {
        ...
        if (AttackTime > 0.5 && this.BoolIsFire) {
            if (this.m_StateManage.m_TargetObj != null && !this.m_StateManage.m_TargetObj.destroyed) {
                ...
                if (!this.m_StateManage.TargetScript.IsDie && DIc <= this.m_StateManage.m_MagicCubeSource.m_TempAttributeBase.m_Range) {
