...
export default class BigSwordStateTHSwordRelax extends CombatantStateBase {
    ...
    public Init(SelfObj: Laya.Sprite3D, _CombatantManageBase: CombatantManageBase)                  //初始化虚函数
    {
        ...
        this.m_EntityState = EntityState.Vertigo;
    }
    public OnEnter()               //进入状态的虚函数
    {
        this.PlayAnimation(GameBigSwordAnimationDefin.AnimationName_THRun);
    ...
    {
        //当前状态不是 这个脚本的时候 就退出
        if ((this.m_StateManage.m_CurStateBase.m_EntityState != EntityState.Vertigo)) {
            return;
        }
    }
