import LoadSubpackagesConfig, { CDNSubpackagesConfig, LoadSubpackagesConfig2 } from "src/Config/SubpackagesConfig";
...
export default class GameInitLoad extends BaseInitLoad {
  ...
  */
    protected loadBefore() {
        ...
        EssentialResUrls.setPrefabSceneNames(_EAllScenePrefabsNames);
        //修改资源路径
        for (let _o of LoadSubpackagesConfig2.subpackages) {
            KeyResManager.editKeyResList(_o.name, _o.root);
        }

        for (let _o of CDNSubpackagesConfig.CDNURLs) {
            KeyResManager.editKeyResList(_o.name, _o.root);
        }
    }
    ...
    private immitFGUiLoadItems(_loadItems: ResLoadItem[]) {
        ...
        _loadItems.push(this.getFGUILoadItems('GameMain', 9));//主包
    }
    //fgui绑定
    private FGUIBinder() {
